const AddEnvMenu = ({ addEnvMenuIsOpen, buttonTriggered, input }) => {
    if (addEnvMenuIsOpen) {
        return (
            <>
                <input
                ...
                <button
                    disabled={input === "" || buttonTriggered}
                    onClick={() => this.save()}
                    className={(input === "" || buttonTriggered ? 'bg-green-600 hover:bg-green-500 focus:outline-none focus:border-green-700 focus:shadow-outline-indigo active:bg-green-700' : `bg-gray-600 cursor-default`) + ` inline-flex items-center px-6 py-3 border border-transparent text-base leading-6 font-medium rounded-md text-white transition ease-in-out duration-150`}>
                    Save
                </button>
            </>
        )
